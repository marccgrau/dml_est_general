hyperparam_lasso_oc = function(y, x) {
  ...
  lambda_cv = cv.glmnet(x, y, nfolds = 10,family = "gaussian", alpha = 1)$lambda.1se
  
  seq_lambda_final = if(lambda_cv - 0.05 < 0) {seq(0, lambda_cv + 0.05, 0.001)} else{seq(lambda_cv - 0.05, lambda_cv + 0.05, 0.001)}
  
  args = list("lambda" = seq_lambda_final, "family" = "gaussian", "alpha" = 1)
