...
single_layer_params = list(
  ...
  act.fct1 = c("sigmoid"),
  act.fct2 = c("relu"),
  learningrate = c(1, 0.1, 0.01, 0.001)
)
...
for (j in 1:nrow(grid_frame_single_layer)){
  ...
  for (i in 1:reps){
    ...
    output <- input %>% 
      ...
      layer_dense(units = grid_frame_single_layer$units2[j], activation = grid_frame_single_layer$act.fct2[j])%>%
      layer_dense(units = 1)
    ...
    model %>% 
      compile(
        ...
        optimizer = optimizer_adam(lr = grid_frame_single_layer$learningrate[j]),
    ...
    single_layer_res[j,2] = average_RMSE
