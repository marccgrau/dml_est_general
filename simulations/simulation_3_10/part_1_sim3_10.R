...
folder = "output/sim_3_10" # set folder to store values
...
db3_10 = dbConnect(SQLite(), dbname = file.path(folder, "db3_10"))

# Hyperparameter Tuning for DGP 1
hyperparams = hyperparam_tuning_3_10(n_covariates, n_observations, mu3, tau3, pi3, sigma = 1, cv_folds, smalltreat = TRUE)
...
for (j in 1:1) {
  ...
  dbWriteTable(conn = db3_10, name = "average_te", value = as.data.table(output_ate), row.names = FALSE, header = TRUE,
               overwrite = TRUE, append = FALSE)
  
  # treatment effects
  for (i in 1:length(ml_methods)){
    temp = as.data.table(get(paste0("te_", ml_methods[i])))
    dbWriteTable(conn = db3_10, name = paste0("treatmenteffect_", ml_methods[i]), value = temp, row.names = FALSE, header = FALSE,
                 overwrite = TRUE, append = FALSE)
    rm(temp)
  }
  
  # standard errors of score
  dbWriteTable(conn = db3_10, name = "standerror_te", value = as.data.table(output_se_te), row.names = FALSE, header = TRUE,
               overwrite = TRUE, append = FALSE)
  dbWriteTable(conn = db3_10, name = "standerror_po", value = as.data.table(output_se_po), row.names = FALSE, header = TRUE,
               overwrite = TRUE, append = FALSE)
  
  # Each ensembles weights
  dbWriteTable(conn = db3_10, name = "propensity_ensemble", value = as.data.table(t(ps_ensemble)), row.names = FALSE, header = TRUE,
               overwrite = TRUE, append = FALSE)
  dbWriteTable(conn = db3_10, name = "outcome_ensemble", value = as.data.table(t(oc_ensemble)), row.names = FALSE, header = TRUE,
               overwrite = TRUE, append = FALSE)
  
  # estimated values and true values for treatment effects, propensity score and outcome
  dbWriteTable(conn = db3_10, name = "y_ensemble", value = as.data.table(y_ens), row.names = FALSE, header = FALSE,
               overwrite = TRUE, append = FALSE)
  dbWriteTable(conn = db3_10, name = "p_ensemble", value = as.data.table(p_ens), row.names = FALSE, header = FALSE,
               overwrite = TRUE, append = FALSE)
  attr(true_p, "dim") <- c(1, n_observations)
  dbWriteTable(conn = db3_10, name = "p_true", value = as.data.table(p_true), row.names = FALSE, header = FALSE,
               overwrite = TRUE, append = FALSE)
  attr(true_te, "dim") <- c(1, n_observations)
  dbWriteTable(conn = db3_10, name = "te_true", value = as.data.table(te_true), row.names = FALSE, header = FALSE,
               overwrite = TRUE, append = FALSE)
  
  # confidence intervals
  for (i in 1:length(ml_methods)){
    temp = as.data.table(t(c(get(paste0("CI_up_", ml_methods[i])))))
    dbWriteTable(conn = db3_10, name = paste0("CI_up_", ml_methods[i]), value = temp, row.names = FALSE, header = FALSE,
                 overwrite = TRUE, append = FALSE)
    rm(temp)
  }
  for (i in 1:length(ml_methods)){
    temp = as.data.table(t(c(get(paste0("CI_down_", ml_methods[i])))))
    dbWriteTable(conn = db3_10, name = paste0("CI_down_", ml_methods[i]), value = temp, row.names = FALSE, header = FALSE,
                 overwrite = TRUE, append = FALSE)
    rm(temp)
  }
  
  print(paste("Simulation 3, 10/90, round: ", j))
  rm(dml_estimator)
