dml_est_cf = function(y, d, x, ps_methods, oc_methods) {
  ...
  if (binary){
    ...
    p_fit_main = regression_forest(x_main, d_main) # fit model d ~ x with main data
    p_fit_aux = regression_forest(x_aux, d_aux) # fit model d ~ x with auxiliary data
    
    p_hat_aux[,1] = predict(p_fit_main, newdata = x_aux)$predictions # predict the propensity score with main model, aux data
    p_hat_aux[,2] = 1 - p_hat_aux[,1] # retrieve 1-p(x) for main model and aux data
    
    p_hat_main[,1] = predict(p_fit_aux, newdata = x_main)$predictions # predict the propensity score with aux model, main data
    ...
    for (i in 1:2){
      y_fit_main = regression_forest(x_main[treatment_main[,i] == 1, ], y_main[treatment_main[,i] == 1]) # fit potential outcome model for both treated and controls
      y_hat_aux[,i] = predict(y_fit_main, newdata = x_aux)$predictions # predict potential outcomes y_hat for treated and controls
    
      y_fit_aux = regression_forest(x_aux[treatment_aux[,i] == 1, ], y_aux[treatment_aux[,i] == 1]) # fit potential outcome model for both treated and controls
      y_hat_main[,i] = predict(y_fit_aux, newdata = x_main)$predictions
    }
