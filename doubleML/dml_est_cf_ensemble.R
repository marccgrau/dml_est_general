dml_est_cf_ensemble = function(y, d, x, ps_methods, oc_methods) {
  ...
  if (binary){
    ...
    p_hat_aux[,1] = ensemble(ps_methods, x = x_main, y = d_main, xnew = x_aux)$fit_full$predictions # predict the propensity score with main model, aux data
    p_hat_aux[,2] = 1 - p_hat_aux[,1] # retrieve 1-p(x) for main model and aux data
    
    p_hat_main[,1] = ensemble(ps_methods, x = x_aux, y = d_aux, xnew = x_main)$fit_full$predictions # predict the propensity score with aux model, main data
    ...
    for (i in 1:2){
      y_hat_aux[,i] = ensemble(oc_methods, x = x_main[treatment_main[,i] == 1, ], y = y_main[treatment_main[,i] == 1, ], xnew = x_aux)$fit_full$predictions
      
      y_hat_main[,i] = ensemble(oc_methods, x = x_aux[treatment_aux[,i] == 1, ], y = y_aux[treatment_aux[,i] == 1, ], xnew = x_main)$fit_full$predictions
    }
